...
batterOBS = []
pitcherWHIP = []
for filename in glob.glob('stats/batting_stats_*.csv'):
    batterData1 = np.genfromtxt(filename, dtype='str', delimiter=',', usecols=(2,21), skip_header=1)
    for row in batterData1:
        name = row[0].lower()
        name = name.replace('.','')
        name = name.replace(' ','')
        name = name.replace('!','')
        row[0] = name
    batterOBS.append(batterData1)
    
for filename in glob.glob('stats/pitching_stats_*.csv'):
    pitcherData1 = np.genfromtxt(filename, dtype='str', delimiter=',', usecols=(2,29), skip_header=1)
    for row in pitcherData1:
        name = row[0].lower()
        name = name.replace('.','')
        name = name.replace(' ','')
        name = name.replace('!','')
        row[0] = name
    pitcherWHIP.append(pitcherData1)
...
x = teamWins[t+1]/numGames[t+1]
y0 = net_degrees[0][t]
y1 = net_degrees[1][t]
plt.figure(figsize=([8,6]))
plt.scatter(x, y0, marker='s', label='Batter OBS')
plt.scatter(x, y1, marker='o', label='Pitcher WHIP')
...
plt.xlabel('Regular season win percentage (2016)', fontsize=16)
...
for y in range(9):
    for i in range(3):
        num = np.sum((teamWins[y+1]-np.mean(teamWins[y+1]))*(net_degrees[i][y]-np.mean(net_degrees[i][y])))/N
        denom = np.std(teamWins[y+1])*np.std(net_degrees[i][y])
        r_xy[i][y] = num / denom
...
plt.figure(figsize=([8,6]))
plt.plot(range(2010,2019), r_xy[0], label='Batters')
